...
bool sempUnicoreHashFindAsterisk(SEMP_PARSE_STATE *parse, uint8_t data)
{
    ...
    // ESP_LOGI("semp", "# sempUnicoreHashPreamble buffer: %s", parse->buffer);
    return true;
}
...
{
    // ESP_LOGI("semp", "commandName sempUnicoreHashPreamble");
    SEMP_SCRATCH_PAD *scratchPad = (SEMP_SCRATCH_PAD *)parse->scratchPad;
    parse->crc ^= data;
    ...
    else
    {
        ...
        // ESP_LOGI("semp", "# sempUnicoreHashPreamble command Name : %s", scratchPad->unicoreHash.sentenceName);
...
{
    // ESP_LOGI("semp", "# sempUnicoreHashPreamble");
    SEMP_SCRATCH_PAD *scratchPad = (SEMP_SCRATCH_PAD *)parse->scratchPad;
    if (data != '#')
        return false;
